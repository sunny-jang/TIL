...
var Caller = (function() {
    var execute = function(obj) {
        if(typeof obj !== "object" || null === obj) { // 파라미터 체크 (방어로직)
            console.warn(">> Caller.execute의 파라미터는 배열 혹은 객체만 가능합니다.");
            return;
        }

        for(var key in obj) {
			if(obj.hasOwnProperty(key)) { // 상속된 속성은 패스! (방어로직)
                if(typeof obj[key] === 'function') {
                    console.group("========== " + key + " ==========");
                    try {
                        obj[key]();
                    } catch(e) { // 오류가 나면, 해당 메소드만 에러 출력하고 계속 실행
